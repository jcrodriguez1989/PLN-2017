...
# en el corpus que baje, guarde 350 de los 9158 articulos de la query por que ya 
# pesa 14M el archivo

# downloads scientific papers from PubMedCentral, according to the desired query
def PmcCorpusGenerator(query, outFile="myCorpus.txt"):
    ...
    #xmlStr = urllib.urlopen(completeUrl).read()
...
def getPaper(actId):
    ...
    #xmlStr = urllib.urlopen(downloadUrl).read()
    xmlDoc = minidom.parseString(xmlStr)
